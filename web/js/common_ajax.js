...
function domainChecker($domain,$this)
{
	...
	{
		$this.after('<span class="error" id="'+$this.attr("id")+'_error">Invalid Domain Format</span>');
		$("#domain_checker_btn").attr("disabled", true);
		return;
	}
	$gourl = "domainChecker";
	...
	$.ajax({
		...
		beforeSend: function()
		{
			$this.after('<span id="'+$this.attr("id")+'_result" class="text-primary">Loading......</span>');
		},
	    success: function(data){
			$("#"+$this.attr("id")+"_result").remove();
	    	...
	    	$this.after('<span id="'+$this.attr("id")+'_result" class="'+$class+'">'+$result+'</span>');
