...
if(!isset($_POST['db_user']) || !isset($_POST['type'])){header("location: /admin/share/servers/database?webid=$webid&db=mariadb");}
...
if($type=="MARIADB"){
	...
	if($action=="new")
	{
		...
		if(!$mariadb->addUserAndDB($db_name, $db_user, $db_pass))
		{
			...
			require_once("views/admin/share/servers/database/mariadb/index.php");
			die("");
		}
	...
	header("Location: /admin/share/servers/database?webid=$webid&db=mariadb");
	die("");
