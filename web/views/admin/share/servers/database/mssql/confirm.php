...
if(!isset($_POST['db_user']) || !isset($_POST['type'])){header("location: /admin/share/servers/database?webid=$webid&db=mssql");}
...
if($type=="MSSQL"){
	...
	if($action=="new")
	{
		...
		if(!$mssql->addUserAndDB("2016",$db_name, $db_user, $db_pass))
		{
			...
			require_once("views/admin/share/servers/database/mssql/index.php");
			die("");
		}
	...
	header("Location: /admin/share/servers/database?webid=$webid&db=mssql");
	die("");
