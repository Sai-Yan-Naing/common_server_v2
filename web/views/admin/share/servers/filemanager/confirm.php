...
if($weborigin!=1){
    ...
    $dir2 = "E:\webroot\LocalUser/".$webuser;
}   
// die($_POST['path']);
...
}else if(isset($_POST['foldername']))
{
	...
	echo filepath($dir,$_POST['foldername'],$webid);
	die();
}else if(isset($_GET['download']))
{
	// die($dir.$_GET['download']);
	download($_GET['common_path'],$webuser,$_GET['download']);
...
function open_file($dir,$file_name, $webid)
{
	...
	?>
		...
		<div class="modal-header">
            <button type="button" class="btn btn-success mr-3" id="save_file" file_name="<?= $file_name ?>"  gourl="/admin/share/servers/filemanager/confirm?webid=<?=$webid?>"  webid="<?=$webid?>">Save</button>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
              <span aria-hidden="true">&times;</span>
            </button>
          </div>
          <div class="modal-body">
            <form action="/admin/share/servers/filemanager/confirm?webid=<?=$webid?>"   webid="<?=$webid?>" method="post" id="file_editer">
...
function uncompressed($from, $to)
{
	...
	function filepath($dir,$foldername,$webid)
	{
		...
		<tr>
          <th  class="folder_click" foldername="<?php if($foldername!=null){ echo $foldername.'/'.$value;}else{echo $value;} ?>" style="cursor: pointer;"  gourl="/admin/share/servers/filemanager/confirm?webid=<?=$webid?>"  webid="<?=$webid?>">
        ...
        foreach ($files_list as $key => $value) {
            ?>
            <tr>
              <th class="open_file" style="cursor: pointer;" data-toggle="modal" <?php if (in_array(getFileExt($dir.'/'.$value), $ext)){ echo 'data-target="#open_file"'; } ?> file_name="<?= $value ?>" gourl="/admin/share/servers/filemanager/confirm?webid=<?=$webid?>"  webid="<?=$webid?>">
              ...
              <th class="d-flex justify-content-end" colspan="2">
              	<a href="/admin/share/servers/filemanager/confirm?webid=<?=$webid?>&download=<?=$value?>" class="btn text-success download_file">
