...
if(!isset($_POST['db_user']) || !isset($_POST['type'])){header("location: /share/servers/database?webid=$webid&db=mysql");}
...
if($type=="MYSQL"){
	...
	if($action=="new")
	{
		...
		if(!$mysql->addUserAndDB($db_name, $db_user, $db_pass))
		{
			...
			require_once("views/share/servers/database/mysql/index.php");
			die("");
		}
	...
	header("Location: /share/servers/database?webid=$webid&db=mysql");
	die("");
